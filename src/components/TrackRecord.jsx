...
const TrackRecord = () => {
  ...
  return (
    <section id="achievements" className="py-20 bg-[#111324]/50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {trackRecordData.map((item, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-[#111324]/85 border border-white/5 hover:border-indigo-400/40 hover:shadow-[0_0_30px_-5px_rgba(99,102,241,0.2)] transition-all duration-300 shadow-lg flex flex-col justify-between group hover:-translate-y-1"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl bg-[#16192e]/90 border border-white/10 flex items-center justify-center group-hover:scale-105 transition-transform">
                    {getIcon(item.icon)}
                  </div>
                  <span className="text-[11px] font-mono px-2.5 py-1 rounded-full bg-indigo-600/15 text-indigo-300 border border-indigo-500/25">
                    {item.badge}
                  </span>
                </div>
