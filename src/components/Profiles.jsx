...
const Profiles = () => {
  ...
  return (
    <section id="profiles" className="py-12 bg-[#07050f]/60 border-y border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {codingProfiles.map((profile) => (
            <a
              ...
              className="group relative p-4 rounded-xl bg-[#0e0a20]/70 hover:bg-[#0e0a20] border border-white/5 hover:border-rose-400/40 hover:shadow-[0_0_30px_-5px_rgba(244,63,94,0.2)] transition-all duration-300 shadow-md hover:-translate-y-1 block"
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-lg bg-[#16102f]/80 border border-white/10 flex items-center justify-center group-hover:scale-105 transition-transform">
                    {getIcon(profile.icon)}
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-slate-100 group-hover:text-rose-400 transition-colors flex items-center gap-1.5">
                      {profile.name}
